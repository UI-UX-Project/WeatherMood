...
class AppLoading extends React.Component<AppLoadingProps> {
  ...
  componentDidMount() {
    ...
    // ); // we're using this implementation for now
  ...
  private async startLoadingAppResourcesAsync() {
    if (!('startAsync' in this.props)) return;

    if (!('onFinish' in this.props)) throw new Error('AppLoading onFinish prop is required if startAsync is provided');

    if (!('onError' in this.props)) throw new Error('AppLoading onError prop is required if startAsync is provided');
